...
local Color = Color
SKIN = {}
SKIN.PrintName = "Billions"
SKIN.Author = "Team Pilcrow"
SKIN.DermaVersion = 1
SKIN.GwenTexture = Material("gwenskin/GModDefault.png")
--changed primary colors of the SKIN
SKIN.bg_color = Color(0, 0, 204, 255)
SKIN.bg_color_sleep = Color(0, 0, 153, 255)
SKIN.bg_color_dark = Color(0, 0, 102, 255)
SKIN.bg_color_bright = Color(0, 0, 255, 255)
SKIN.frame_border = Color(0, 0, 0, 255)
--font type for the frame??
SKIN.fontFrame = "DermaDefault"
--changed values for blue colors
SKIN.control_color = Color(0, 128, 255, 255)
SKIN.control_color_highlight = Color(77, 166, 255, 255)
SKIN.control_color_active = Color(51, 153, 255, 255)
SKIN.control_color_bright = Color(102, 179, 255, 255)
SKIN.control_color_dark = Color(0, 102, 204, 255)
SKIN.bg_alt1 = Color(0, 0, 230, 255)
SKIN.bg_alt2 = Color(0, 0, 210, 255)
...
--red color for highlight, gotta change it to sth else
...
--changed here too
SKIN.colPropertySheet = Color(0, 0, 204, 255)
SKIN.colTab = SKIN.colPropertySheet
SKIN.colTabInactive = Color(0, 0, 153, 255)
...
function SKIN:PaintPanel(panel, w, h)
	...
	--draw an outline for some panels for visibility
	if (panel:GetParent() and panel:GetParent():GetName() == "DPanel") or (panel:GetParent() and panel:GetParent():GetParent() and panel:GetParent():GetParent():GetName() == "DScrollPanel") or (panel:GetParent() and panel:GetParent():GetName() == "DIconLayout") then
		draw.RoundedBox(0, 0, 0, w, h, Color(90, 90, 95, 255))
		draw.RoundedBox(0, 0 + 1, 0 + 1, w - 2, h - 2, Color(70, 70, 84, 255))
	else
		draw.RoundedBox(0, 0, 0, w, h, Color(70, 70, 84, 255))
	end
...
function SKIN:PaintFrame(panel, w, h)
	draw.RoundedBox(0, 0, 0, w, h, Color(36, 36, 41, 255))
end
...
function SKIN:PaintButton(panel, w, h)
	...
	if (panel.Depressed or panel:IsSelected() or panel:GetToggle()) and panel:GetName() != "PCNavIcon" then
		draw.RoundedBox(0, 0 + 1, 0 + 1, w - 2, h - 2, Color(33, 33, 34, 255))
	elseif (panel:GetDisabled()) and panel:GetName() != "PCNavIcon" then
		draw.RoundedBox(0, 0 + 1, 0 + 1, w - 2, h - 2, Color(186, 186, 197, 255))
	elseif panel.Hovered and panel:GetName() != "PCNavIcon" then
		draw.RoundedBox(0, 0, 0, w, h, Color(90, 90, 95, 255))
		draw.RoundedBox(0, 0 + 1, 0 + 1, w - 2, h - 2, Color(36, 36, 41, 255))
	elseif panel:GetName() != "PCNavIcon" then
		draw.RoundedBox(0, 0 + 1, 0 + 1, w - 2, h - 2, Color(48, 48, 54, 255))
	else
		draw.RoundedBox(0, 0 + 1, 0 + 1, w, h - 2, Color(48, 48, 54, 255))
	end
	panel:SetFGColor(255, 255, 255, 255)
end
...
function SKIN:PaintTextEntry(panel, w, h)
	panel:SetFont("FrameTitleFontSmall")
	if (panel:GetDisabled()) then
		draw.RoundedBox(0, 0, 0, w, h, Color(90, 90, 95, 255))
		draw.RoundedBox(0, 0 + 1, 0 + 1, w - 2, h - 2, Color(130, 130, 151, 255))
	elseif (panel:HasFocus()) then
		draw.RoundedBox(0, 0, 0, w, h, Color(90, 90, 95, 255))
		draw.RoundedBox(0, 0 + 1, 0 + 1, w - 2, h - 2, Color(47, 47, 55, 255))
	else
		draw.RoundedBox(0, 0, 0, w, h, Color(90, 90, 95, 255))
		draw.RoundedBox(0, 0 + 1, 0 + 1, w - 2, h - 2, Color(59, 59, 69, 255))
	end
...
function SKIN:PaintPropertySheet(panel, w, h)
	draw.RoundedBox(0, 0, 0, w, h, Color(90, 90, 95, 255))
	draw.RoundedBox(0, 0 + 1, 0 + 1, w - 2, h - 2, Color(50, 50, 61, 255))
end
...
function SKIN:PaintTab(panel, w, h)
	panel:SetFont("FrameTabFont")
	if (panel:IsActive()) then return self:PaintActiveTab(panel, w, h) end
	--draw.RoundedBox(0, 0 + 1, 0 + 1 , w - 2, h - 2, Color(50, 50, 61, 255))
	draw.RoundedBox(0, 0, 0, w, h, Color(90, 90, 95, 255))
	draw.RoundedBox(0, 0 + 1, 0 + 1, w - 2, h - 2, Color(34, 34, 42, 255))
	--self.tex.TabT_Inactive(0, 0, w, h, Color(50, 50, 61, 255))

end

function SKIN:PaintActiveTab(panel, w, h)
	draw.RoundedBox(0, 0, 0, w, h, Color(90, 90, 95, 255))
	draw.RoundedBox(0, 0 + 1, 0 + 1, w - 2, h - 1, Color(50, 50, 61, 255))

	--draw.RoundedBox(0, 0, 0, w, h, Color(90, 90, 95, 255))
	--draw.RoundedBox(0, 0 + 1, 0 + 1, w - 2, h - 2, Color(50, 50, 61, 255))
	--self.tex.TabT_Active(0, 0, w, h, Color(50, 50, 61, 255))
end
...
function SKIN:PaintWindowCloseButton(panel, w, h)
	if (panel.Hovered) then
		draw.RoundedBox(0, 0, 0, w, h, Color(255, 77, 77, 255))
	end

	surface.SetFont("WindowCloseButtonFont")
	local textW, textH = surface.GetTextSize("X")
	draw.SimpleText("X", "WindowCloseButtonFont", (w / 2) - (textW / 2), (h / 2) - (textH / 2), Color(255, 255, 255, 255))
end
...
function SKIN:PaintComboBox(panel, w, h)
	if (panel:GetDisabled()) then return self.tex.Input.ComboBox.Disabled(0, 0, w, h, Color(130, 130, 151, 255)) end
	if (panel.Depressed or panel:IsMenuOpen()) then return self.tex.Input.ComboBox.Down(0, 0, w, h, Color(130, 130, 151, 255)) end
	if (panel.Hovered) then return self.tex.Input.ComboBox.Hover(0, 0, w, h, Color(58, 58, 69, 255)) end
	self.tex.Input.ComboBox.Normal(0, 0, w, h, Color(58, 58, 69, 255))
	panel:SetFGColor(255, 255, 255, 255)
end
...
function SKIN:PaintCollapsibleCategory(panel, w, h)
	panel:GetChild(0):SetFont("FrameTextFont")
	if h < 21 then
		draw.RoundedBox(0, 0, 0, w, h, Color(90, 90, 95, 255))
		draw.RoundedBox(0, 0 + 1, 0 + 1, w - 2, h - 2, Color(34, 34, 42, 255))
	else
	...
	--if (h < 21) then return self.tex.CategoryList.Header(0, 0, w, h, Color(59, 59, 69, 255)) end
	--self.tex.CategoryList.Inner(0, 0, w, 63, Color(59, 59, 69, 255))
	panel:SetFGColor(255, 255, 255, 255)
end
...
function SKIN:PaintCategoryList(panel, w, h)
	draw.RoundedBox(0, 0, 0, w, h, Color(90, 90, 95, 255))
	draw.RoundedBox(0, 0 + 1, 0 + 1, w - 2, h - 2, Color(70, 70, 84, 255))
	--self.tex.CategoryList.Outer(0, 0, w, h, Color(81, 81, 97, 255))
	panel:SetFGColor(255, 255, 255, 255)
end

function SKIN:PaintCategoryButton(panel, w, h)
	panel:SetFont("FrameTextFont")
	--draw.RoundedBox(0, 0, 0, w, h, Color(90, 90, 95, 255))
	--draw.RoundedBox(0, 0, 0, w, h - 1, Color(70, 70, 84, 255))
	local color = Color(70, 70, 84, 255)
	if (panel.AltLine) then
		if (panel.Depressed or panel.m_bSelected) then
			color = Color(159, 159, 173, 255)
		elseif (panel.Hovered) then
			color = Color(130, 130, 151, 255)
		else
			color = Color(81, 81, 97, 255)
		end
	else
		if (panel.Depressed or panel.m_bSelected) then
			color = Color(159, 159, 173, 255)
...
function SKIN:PaintListViewLine(panel, w, h)
	panel:GetChild(0):SetTextColor(Color(255, 255, 255, 255))
	panel:GetChild(0):SetFont("FrameListViewColumnFont")

	panel:GetChild(1):SetFont("FrameListViewColumnFont")
...
function SKIN:PaintListView(panel, w, h)
	if (not panel.m_bBackground) then return end
	self.tex.Input.ListBox.Background(0, 0, w, h, Color(81, 81, 97, 255))
end
...
derma.DefineSkin("Billions", "Billions default skin", SKIN)
